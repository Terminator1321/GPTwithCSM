...
{
    if (axis >= shape_.size()) throw std::out_of_range("dim(): axis out of range");
    return shape_[axis];
}
...
{
    if (shape_.size() > 1) return shape_[1];
...
{
    if (!grad_) throw std::runtime_error("grad(): gradient buffer not allocated (requires_grad is false)");
    if (!is_contiguous() || offset_ != 0) {
        throw std::runtime_error("grad(): tensor is a non-contiguous/offset view");
    }
    return *grad_;
...
size_t Tensor::flat_offset(const std::vector<size_t>& index) const
{
    ...
    for (size_t i = 0; i < index.size(); ++i) {
        if (index[i] >= shape_[i]) throw std::out_of_range("index out of bounds");
...
{
    if (is_contiguous()) return (*data_)[offset_ + i];
...
{
    if (shape_.size() < 2) throw std::runtime_error("2D access used on a rank<2 tensor");
    return at({row, col});
}
...
void Tensor::random(double mean, double stddev)
{
    ...
    for (size_t i = 0; i < size(); ++i) (*this)(i) = dist(gen);
}
...
{
    if (axis >= shape_.size()) throw std::out_of_range("slice(): axis out of range");
    if (start >= end || end > shape_[axis]) throw std::out_of_range("slice(): invalid range");
...
{
    if (is_contiguous() && offset_ == 0) return *this;
    return clone();
}
...
Tensor Tensor::divide(const Tensor& a, const Tensor& b)
{
    ...
    for (size_t linear = 0; linear < total; ++linear) {
        ...
        if (denom == 0.0) throw std::runtime_error("divide(): division by zero");
...
Tensor Tensor::sum(std::optional<size_t> axis) const
{
    if (!axis.has_value()) {
        double total = 0.0;
        for (size_t i = 0; i < size(); ++i) total += (*this)(i);
        Tensor out({1});
        out(0) = total;
        return out;
    }
    ...
    if (ax >= shape_.size()) throw std::out_of_range("sum(): axis out of range");
...
Tensor Tensor::max(std::optional<size_t> axis) const
{
    if (!axis.has_value()) {
        double best = (*this)(0);
        for (size_t i = 1; i < size(); ++i) best = std::max(best, (*this)(i));
        Tensor out({1});
        out(0) = best;
        return out;
    }
    ...
    size_t ax = axis.value();
    if (ax >= shape_.size()) throw std::out_of_range("max(): axis out of range");
    ...
    for (size_t linear = 0; linear < total; ++linear) {
        ...
        out_idx[ax] = 0;
        double v = at(idx);
        if (v > out.at(out_idx)) out.at(out_idx) = v;
    }
    return out;
}
...
Tensor Tensor::argmax(std::optional<size_t> axis) const
{
    if (!axis.has_value()) {
        ...
        for (size_t i = 1; i < size(); ++i) {
            if ((*this)(i) > best) { best = (*this)(i); best_idx = i; }
        }
        Tensor out({1});
    ...
    size_t ax = axis.value();
    if (ax >= shape_.size()) throw std::out_of_range("argmax(): axis out of range");
...
{
    if (shape_.size() == 1) {
        for (size_t i = 0; i < shape_[0]; ++i) std::cout << std::setw(10) << (*this)(i) << " ";
        std::cout << std::endl;
        return;
    }
    if (shape_.size() == 2) {
        for (size_t r = 0; r < rows(); ++r) {
            for (size_t c = 0; c < cols(); ++c) std::cout << std::setw(10) << (*this)(r, c) << " ";
            std::cout << std::endl;
        }
        return;
    }
    std::cout << "Tensor(shape=[";
    for (size_t i = 0; i < shape_.size(); ++i) std::cout << shape_[i] << (i + 1 < shape_.size() ? "," : "");
