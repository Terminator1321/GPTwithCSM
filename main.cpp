...
int main()
{
    ...
    for (int epoch = 1; epoch <= epochs; ++epoch)
    {
        ...
        for (const auto &seq : train_sequences)
        {
            ...
            {
                if (saveCheckpoint(checkpoint_path, gpt, global_step, tokenizer.totalTokens, gpt_embed_dim, gpt_max_seq_len, gpt_num_heads, gpt_num_layers))
                {
                    std::cout << "  [checkpoint] saved '" << checkpoint_path << "' at step " << global_step << std::endl;
                }
            }
        }
        ...
        std::cout << "epoch " << std::setw(2) << epoch << "/" << epochs << " | train_loss=" << std::fixed << std::setprecision(4) << epoch_train_loss << " | val_loss=";
    ...
    if (saveCheckpoint(checkpoint_path, gpt, global_step, tokenizer.totalTokens, gpt_embed_dim, gpt_max_seq_len, gpt_num_heads, gpt_num_layers))
    {
        std::cout << "Final checkpoint saved to '" << checkpoint_path << "' (step " << global_step << ")." << std::endl;
    }
